...
public class PictureController {
    ...
    private final Cache<String, String> LOCAL_CACHE =
            Caffeine.newBuilder().initialCapacity(1024)
                    ...
                    .build(); //随用随更新 不需要更新策略了

    /**
     * 上传图片（可重新上传）
     * @RequestPart("file") MultipartFile multipartFile 意思是需要单独以form-data格式传递文件
     * PictureUploadRequest 允许传日图片id，用于编辑图片
     */
    @PostMapping("/upload")
    ...
    public BaseResponse<PictureVO> uploadPicture(
        ...
        User loginUser = userService.getLoginUser(request); //用于判断是否登录
    ...
    public BaseResponse<Page<PictureVO>> listPictureVOByPageWithCache(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        //这里用md5哈希算法把 查询条件长字符串 压缩转为hash key
        ...
        // 更新redis缓存，5 - 10 分钟随机过期，防止雪崩 （击穿，一致性问题）
    ...
    public BaseResponse<List<ImageSearchResult>> searchPictureByPicture(@RequestBody SearchPictureByPictureRequest searchPictureByPictureRequest) {
        ...
        ThrowUtils.throwIf(oldPicture == null, ErrorCode.NOT_FOUND_ERROR);
        // 因为百度识图搜索不支持webp格式，因此使用COS自带方法转换为其他格式
